...
from utility import str_gt, draw_topo_graph
...
def build_jellyfish_graph(num_racks, switch_degree, num_hosts_per_rack):
    ...
    for i in range(1, num_racks + 1):
        switch_set.add('s%s' % i)
        graph.add_node('s%s' % i, type='switch')
    
    host_set = set()
    for i in range(1, num_hosts_per_rack*num_racks + 1):
        host_set.add('h%s' % i)
    ...
    for i in range(0, num_racks):
        for j in range(1, num_hosts_per_rack + 1):
            graph.add_edge('s%s' % (i + 1), 'h%s' % (i * num_hosts_per_rack + j))
...
def build_jellyfish_graph_identical_equipment_as_fattree(num_racks, num_racks_with_hosts, switch_degree, num_hosts_per_rack):
    """
    Uses the networkx library to build a JellyFish-like topology with identical equipment as fattree.
    e.g. build_jellyfish_graph_identical_equipment_as_fattree(20, 16, 4, 1) produces figure 1.b
    ...
    host_set = []
    for i in range(1, num_hosts_per_rack*num_racks_with_hosts + 1):
...
if __name__ == "__main__":
    # draw figure 1.a
    fattree_4 = build_fattree_graph(4)
    draw_topo_graph(fattree_4, outfile='./figure 1.a.png')

    # draw figure 1.b
    jellyfish_like = build_jellyfish_graph_identical_equipment_as_fattree(20, 16, 4, 1)
    draw_topo_graph(jellyfish_like, outfile='./figure 1.b.png')
